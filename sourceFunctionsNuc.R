...
initializeLexicon<-function(r_n,envir=lexicon)
{
  ...
  for(i in 1:length(alph[[1]]))
  {
    ...
    assign(as.character(alph[[1]][i]),c(obs,Expected=1/r_n,Enrichment=obs$Observed/(obs$Observed+1/r_n)),envir=lexicon)
  }
...
{
  inst<-SYMDATA[,gregexpr2(qword,symSignal),by=chrName]
  inst<-inst[V1!=-1]
...
getExpectedFreq<-function(qword,envir=lexicon){
  ...
  for(j in 1:length(Parts)){
    ...
    {
      qq<-names(tbl)[k]
      Pobs<-get(qq,envir=lexicon)$Observed
      Ppart<-Ppart*Pobs^tbl[[k]]
      # Pp<-Pp*ngramFreq[,Frequency[Ngram==nchar(qq)]]
    }
    # Ppsum<-Ppsum+exp(log(Pp)+length(Parts[[j]])*log(lngt[,sum(V1)]))
    PpartTot[j]<-Ppart
    PpTot[j]<-Pp
  }
  
  Pexp<-sum(PpartTot*PpTot/Ppsum)
